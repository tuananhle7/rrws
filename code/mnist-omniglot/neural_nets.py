...
class GenerativeNetworkL1(nn.Module):
    def __init__(self, train_observation_bias):
        ...
        self.latent_logalpha = nn.Parameter(torch.zeros(200))
    ...
    def get_latent_params(self):
        return torch.exp(-softplus(-self.latent_logalpha))

    def get_latent_logalpha(self):
        return self.latent_logalpha
    ...
    def get_observation_params(self, latent):
        return torch.exp(-softplus(-self.get_observation_logalpha(latent)))
...
class InferenceNetworkL1(nn.Module):
    ...
    def get_latent_params(self, observation):
        return torch.exp(-softplus(-self.get_latent_logalpha(observation)))
...
class GenerativeNetworkNL(nn.Module):
    def __init__(self, train_observation_bias):
        super(GenerativeNetworkNL, self).__init__()
        self.latent_logalpha = nn.Parameter(torch.zeros(200))
    ...
    def get_observation_logalpha(self, latent):
        return self.nl(latent) + self.train_observation_bias
...
class InferenceNetworkNL(nn.Module):
    def __init__(self, train_observation_mean):
        super(InferenceNetworkNL, self).__init__()
        self.nl = nn.Sequential(
            nn.Linear(784, 200),
            nn.Tanh(),
            nn.Linear(200, 200),
    ...
    def get_latent_logalpha(self, observation):
        return self.nl(observation - self.train_observation_mean)
...
class RelaxControlVariateNL(nn.Module):
    def __init__(self, train_observation_mean):
        super(RelaxControlVariateNL, self).__init__()
        self.log_temperature = nn.Parameter(torch.log(torch.Tensor([0.5])))
        self.surrogate = nn.Sequential(
            nn.Linear(200 + 784, 200),
            nn.ReLU(),
...
class VAENL(nn.Module):
    def __init__(self, estimator, train_observation_mean, train_observation_bias):
        super(VAENL, self).__init__()
        self.inference_network = InferenceNetworkNL(train_observation_mean)
        self.generative_network = GenerativeNetworkNL(train_observation_bias)
        self.vae_params = list(self.inference_network.parameters()) + list(self.generative_network.parameters())
        self.estimator = estimator
        if self.estimator == 'relax':
            self.control_variate = RelaxControlVariateNL(train_observation_mean)
...
class GenerativeNetworkL2(nn.Module):
    def __init__(self, train_observation_bias):
        super(GenerativeNetworkL2, self).__init__()
        self.latent_2_logalpha = nn.Parameter(torch.zeros(200))
    ...
    def get_latent_2_logalpha(self):
        return self.latent_2_logalpha

    def get_latent_2_params(self):
        return torch.exp(-softplus(-self.get_latent_2_logalpha()))

    def get_latent_1_logalpha(self, latent_2):
        return self.lin2(latent_2)

    def get_latent_1_params(self, latent_2):
        return torch.exp(-softplus(-self.get_latent_1_logalpha(latent_2)))

    def get_latent_log_density(self, latent):
        latent_1, latent_2 = latent
        latent_2_logalpha = self.get_latent_2_logalpha().unsqueeze(0).expand_as(latent_2)
        latent_1_logalpha = self.get_latent_1_logalpha(latent_2)
        return torch.sum(
            bernoulli_logprob((latent_1 + 1) / 2, latent_1_logalpha),
            dim=1
        ) + torch.sum(
            bernoulli_logprob((latent_2 + 1) / 2, latent_2_logalpha),
            dim=1
        )

    def get_observation_logalpha(self, latent):
        latent_1, latent_2 = latent
        return self.lin1(latent_1) + self.train_observation_bias
    ...
    def sample_latent(self, num_samples):
        latent_2_params = self.get_latent_2_params().unsqueeze(0).expand(num_samples, -1)
        latent_2 = torch.distributions.Bernoulli(latent_2_params).sample().float()
        latent_1_params = self.get_latent_1_params(latent_2)
        latent_1 = torch.distributions.Bernoulli(latent_1_params).sample().float()
...
class InferenceNetworkL2(nn.Module):
    def __init__(self, train_observation_mean):
        super(InferenceNetworkL2, self).__init__()
        self.lin1 = nn.Linear(784, 200)
        self.lin2 = nn.Linear(200, 200)
        self.train_observation_mean = train_observation_mean
    ...
    def get_latent_1_logalpha(self, observation):
        return self.lin1(observation - self.train_observation_mean)

    def get_latent_2_logalpha(self, latent_1):
        return self.lin2(latent_1)

    def get_latent_1_params(self, observation):
        return torch.exp(-softplus(-self.get_latent_1_logalpha(observation)))

    def get_latent_2_params(self, latent_1):
        return torch.exp(-softplus(-self.get_latent_2_logalpha(latent_1)))

    def get_latent_log_density(self, observation, latent):
        latent_1, latent_2 = latent
        latent_1_logalpha = self.get_latent_1_logalpha(observation)
        latent_2_logalpha = self.get_latent_2_logalpha(latent_1)
        return torch.sum(
            bernoulli_logprob((latent_1 + 1) / 2, latent_1_logalpha),
            dim=1
        ) + torch.sum(
    ...
    def sample_latent_1(self, observation):
        latent_1_params = self.get_latent_1_params(observation)
        return torch.distributions.Bernoulli(latent_1_params).sample().float() * 2 - 1

    def sample_latent_2(self, latent_1):
        latent_2_params = self.get_latent_2_params(latent_1)
        return torch.distributions.Bernoulli(latent_2_params).sample().float() * 2 - 1

    def sample_latent(self, observation):
        latent_1 = self.sample_latent_1(observation)
        latent_2 = self.sample_latent_2(latent_1)
        return latent_1, latent_2

    def sample_latent_1_aux(self, observation):
        latent_1_params = self.get_latent_1_params(observation)
        u = Variable(torch.rand(200).cuda() if self.is_cuda() else torch.rand(200))
        v = Variable(torch.rand(200).cuda() if self.is_cuda() else torch.rand(200))
        aux_latent_1 = reparam(u, latent_1_params) # z
        latent_1 = heaviside(aux_latent_1).float().detach() # b
        aux_latent_1_tilde = conditional_reparam(v, latent_1_params, latent_1) # z_tilde
        return latent_1 * 2 - 1, aux_latent_1, aux_latent_1_tilde

    def sample_latent_2_aux(self, latent_1):
        latent_2_params = self.get_latent_2_params(latent_2)
        u = Variable(torch.rand(200).cuda() if self.is_cuda() else torch.rand(200))
        v = Variable(torch.rand(200).cuda() if self.is_cuda() else torch.rand(200))
        aux_latent_2 = reparam(u, latent_2_params) # z
        latent_2 = heaviside(aux_latent_2).float().detach() # b
        aux_latent_2_tilde = conditional_reparam(v, latent_2_params, latent_2) # z_tilde
        return latent_2 * 2 - 1, aux_latent_2, aux_latent_2_tilde

    def sample_latent_aux(self, observation):
        latent_1, aux_latent_1, aux_latent_1_tilde = self.sample_latent_1_aux(observation)
        latent_2, aux_latent_2, aux_latent_2_tilde = self.sample_latent_2_aux(observation)
        return (latent_1, aux_latent_1, aux_latent_1_tilde), (latent_2, aux_latent_2, aux_latent_2_tilde)

    def forward(self, generated_latent, generated_observation):
        return -torch.mean(self.get_latent_log_density(generated_observation, generated_latent))


# class RelaxControlVariateL2(nn.Module):
#     def __init__(self, train_observation_mean):
#         super(RelaxControlVariateL2, self).__init__()
#         self.log_temperature = nn.Parameter(torch.log(torch.Tensor([0.5])))
#         self.surrogate = nn.Sequential(
#             nn.Linear(200 + 784, 200),
#             nn.ReLU(),
...
#     def __init__(self, estimator, train_observation_mean, train_observation_bias):
#         super(VAEL2, self).__init__()
#         self.inference_network = InferenceNetworkL2(train_observation_mean)
#         self.generative_network = GenerativeNetworkL2(train_observation_bias)
#         self.vae_params = list(self.inference_network.parameters()) + list(self.generative_network.parameters())
#         self.estimator = estimator
#         if self.estimator == 'relax':
#             self.control_variate = RelaxControlVariateL2(train_observation_mean)
#
#     def is_cuda(self):
#         return next(self.parameters()).is_cuda
#
...
#             return -torch.mean(elbo + elbo.detach() * log_q_latent_density), elbo
#         elif self.estimator == 'relax':
#             (latent_1, aux_latent_1, aux_latent_1_tilde), (latent_2, aux_latent_2, aux_latent_2_tilde) = self.inference_network.sample_latent_aux(observation)
#             c = self.control_variate(aux_latent, observation, self.generative_network, self.inference_network)
#             c_tilde = self.control_variate(aux_latent_tilde, observation, self.generative_network, self.inference_network)
#             log_q_latent_density = self.inference_network.get_latent_log_density(observation, latent)
#             log_p_joint_density = self.generative_network.get_joint_log_density(latent, observation)
